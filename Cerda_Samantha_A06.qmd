...
paste0("The largest win spread is ", max(win_spread), ", when the Bears scored ", bears[max_index], " points and the Opponent scored ", opponent[max_index], " points.")
```
## Question 2 (7 points)
...
```{r}
#no need to name each pair added 
game_score <- rbind(game_score, c(37, 17))
game_score <- rbind(game_score, c(9, 17))
print(game_score)
```
### Part c
...
```{r}
#there are two colunmns to compare
for (i in 1:nrow(game_score)) {
  bears_score <- game_score[i, 1]
  opponent_score <- game_score[i, 2]
  
  if (bears_score > opponent_score) {
    print("Win")
  } else if (bears_score < opponent_score) {
    print("Loss")
  }
}
```
...
```{r}
#only care about wins, so inequality will be > 
#indexing can help us compare both columns
...
```{r}
#like adding new rows from previous problem
...
```{r}
#similar to one in python
number_of_factors <- function(intgr) {
  factor_count <- 0
  for (i in 1:intgr) {
    if (intgr %% i == 0) {
...
rectangle_area <- function(length = 48, width = 48) {
  ...
  area_ft <- area_in / 144 
  #output will be a variable that has both values, so a list
  return(list(inches = area_in, feet = area_ft))
}
```

```{r}
output_variable <- rectangle_area(102, 60)
#slicing a list with $
paste0("The area in square inches is ", output_variable$inches, ".")
paste0("The area in square feet is ", output_variable$feet, ".")
```
## Question 5 (5 points)
...
fruits <- c("apple", "orange", "banana", 
            "pear", "grapes", "kiwi")
most_letters <- function(str_vector) {
  words <- unlist(strsplit(str_vector, " ")) #words split into a list, so unlist since we need a vector
  length <- nchar(words) #counts the number of characters for each word 
  max_length <- max(length) 
  longest_words <- words[length == max_length] #vector made of words with longest length 
...
value <- 2
check <- seq(1, 15, 3)
#any() returns T or F if condition is met at least once 
...
```{r}
logical <- in_vector(val = value, vec = check)

if (logical == TRUE) {
  paste0(value, " is in the vector.")
} else {
  paste0(value, " is not in the vector.")
}
```
## Question 7 (6 points)
...
```{r}
converts_matrix <- function(mtx) {
  mtx[mtx < 0] <- NA #all negative elements become NA 
  return(mtx)
}
```
...
```{r}
even_and_odd <- function(mtx) {
  for (i in 1:nrow(mtx)) {
    for (j in 1:ncol(mtx)) {
      if (mtx[i, j] %% 2 == 0) { #checking element by element
        mtx[i, j] <- 1
      } else {
        mtx[i, j] <- 0 #odd numbers
      }
    }
  }
  return(mtx)
